...
X_train = data_dict["X_train"]  # forme initiale : (470, 3104)
X_test = data_dict["X_test"]
y_train = data_dict["y_train"].astype(np.float32)

print("Forme initiale des données d'entraînement :", X_train.shape)

# Nombre de canaux connu
expected_channels = 6
n_samples, fused_dim = X_train.shape

# Calcul du nombre de points temporels par canal
n_time_steps = fused_dim // expected_channels

# Si fused_dim n'est pas exactement divisible, on tronque les points en trop
X_train = X_train[:, :expected_channels * n_time_steps].reshape(n_samples, expected_channels, n_time_steps)
print("Nouvelle forme des données d'entraînement :", X_train.shape)
...
class CNN1DClassifier(nn.Module):
    def __init__(self, input_channels, output_dim=1):
        ...
        # Après deux max pooling, la dimension temporelle est divisée par 4
...
model = CNN1DClassifier(input_channels=expected_channels).to(device)

# Utiliser BCEWithLogitsLoss (la sigmoïde sera appliquée lors de l'évaluation)
...
num_epochs = 30  # Entraînement sur plusieurs époques
train_losses = []
val_losses = []
